    ...
    if ($_SERVER['REQUEST_METHOD']==='POST') { 
        ...
        if($enviar!=0){
            if ($enviar==1 & $_POST['nomb_i'] !='' ) {
                ...
                echo "---------------------",$nomb_in;
                $consulta=consulta($miconexion,"INSERT INTO `inc_estado`(`nombre_estado`) VALUES ('$nomb_in')");
                if($consulta)
                {  /* -----------------Alerta para notificar registro ------------------------*/
                    echo "<script>
                        ...
                        window.location = 'inc_estado.php';
                    });
                    </script>" ; 
        ...
        if ($enviar==2 & $_POST['nomb_iM1'] !=''& $_POST['nomb_iM'] !='') {
            ...
            $resultado=consulta($miconexion,"SELECT * FROM `inc_estado` WHERE `id_estado` like '$nomb_in' ");
            $fila0=$resultado->fetch_object(); 
            $valor=$fila0->id_estado;

            $consulta=consulta($miconexion,"UPDATE `inc_estado` SET `nombre_estado`='$nomb_in1' WHERE id_estado like '$valor'");
            if($consulta)
            {  /* -----------------Alerta para notificar registro ------------------------*/
                  echo "<script>
                    Swal.fire({type: 'success',
        ...
        if ($enviar==3 & $_POST['nomb_iE'] !='') {
            ...
            $consulta=consulta($miconexion,"DELETE FROM `inc_estado` WHERE  id_estado like '$nomb_in'");
            if($consulta)
            {  /* -----------------Alerta para notificar registro ------------------------*/
                  echo "<script>
                    Swal.fire({type: 'success',
