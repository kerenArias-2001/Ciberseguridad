...
<section id="acuerdo">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="page-header"> Modificaciones para tipo_activo de Incidentes </h2>
                        </div>
                    </div>
                <div class="cont-b">
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST') { 
        ...
        if($enviar!=0){
            if ($enviar==1 & $_POST['nomb_i'] !='' ) {
                ...
                echo "---------------------",$nomb_in;
                $consulta=consulta($miconexion,"INSERT INTO `act_tipo_activo`(`nombre_tipo_activo`) VALUES ('$nomb_in')");
                if($consulta)
                {  /* -----------------Alerta para notificar registro ------------------------*/
                    echo "<script>
                        ...
                        window.location = 'act_tipo_activo.php';
                    });
                    </script>" ; 
        ...
        if ($enviar==2 & $_POST['nomb_iM1'] !=''& $_POST['nomb_iM'] !='') {
            ...
            $resultado=consulta($miconexion,"SELECT * FROM `act_tipo_activo` WHERE `id_tipo_activo` like '$nomb_in' ");
            $fila0=$resultado->fetch_object(); 
            $valor=$fila0->id_tipo_activo;

            $consulta=consulta($miconexion,"UPDATE `act_tipo_activo` SET `nombre_tipo_activo`='$nomb_in1' WHERE id_tipo_activo like '$valor'");
            if($consulta)
            {  /* -----------------Alerta para notificar registro ------------------------*/
                  echo "<script>
                    Swal.fire({type: 'success',
        ...
        if ($enviar==3 & $_POST['nomb_iE'] !='') {
            ...
            $consulta=consulta($miconexion,"DELETE FROM `act_tipo_activo` WHERE  id_tipo_activo like '$nomb_in'");
            if($consulta)
            {  /* -----------------Alerta para notificar registro ------------------------*/
                  echo "<script>
                    Swal.fire({type: 'success',
