...
<head>
    ...
    <title>Modificación escalamiento de incidentes | Ciberseguridad</title>
       <!-- Bootstrap Core CSS -->
...
<center>  
    ...
    <label class="" for="incidente">Agregar nuevo escalamiento de incidente </label><br>
    ...
    <tr  ><td>
    <label class="" for="Mincidente">Modificar escalamiento de incidente </label><br>
    ...
    <label for="Mincidente">ingrese nuevo escalamiento de incidente </label><br>
    ...
    <tr  ><td>
    <label for="Eincidente">Eliminar escalamiento de incidente </label><br>
...
/* 
            echo "---------------------",$nomb_in; */
            $consulta=consulta($miconexion,"INSERT INTO `escalamiento`(`nombre_escalamiento`) VALUES ('$nomb_in')");
        ...
        if ($enviar==2) {
            ...
            $resultado=consulta($miconexion,"SELECT * FROM `escalamiento` WHERE `id_escalamiento` like '$nomb_in' ");
            $fila0=$resultado->fetch_object(); 
            $valor=$fila0->id_escalamiento;

            $consulta=consulta($miconexion,"UPDATE `escalamiento` SET `nombre_escalamiento`='$nomb_in1' WHERE id_escalamiento like '$valor'");
            if($consulta)  {  /* -----------------Alerta para notificar registro ------------------------*/
                echo "<script>
                  Swal.fire({type: 'success',
                  text: 'Guardado Exitoso',
        ...
        if ($enviar==3) {
            ...
            $consulta=consulta($miconexion,"DELETE FROM `escalamiento` WHERE  id_escalamiento like '$nomb_in'");
            if($consulta)  {  /* -----------------Alerta para notificar registro ------------------------*/
                echo "<script>
                  Swal.fire({type: 'success',
                  text: 'Guardado Exitoso',
